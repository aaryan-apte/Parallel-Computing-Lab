...
int main(int argc, char *argv[])
{
    ...
    // Second Example MPI Program

    /* print host name, and send message from
    process with rank 0 to all other processes */
    if (MyRank == 0)
    {
        strcpy(Message, "Hello India");
        for (temp = 1; temp < Numprocs; temp++)
            MPI_Send(Message, BUFFER_SIZE,
                     MPI_CHAR, temp, tag, MPI_COMM_WORLD);
    }
    else
    {
        // system("hostname");
